...
# FileName:     apiModuleController.py
# Author:      Jakilo
# Datetime:    2024/2/22 19:52
...
from common.modules.api_manage.apiModuleModel import ApiModuleModel

# 模块名称X
module_name = "CtApiModule"
module_model: db.Model = ApiModuleModel
...
def query_api_module_by_page():
    try:
        ...
        with application.app_context():
            ...
            filter_list = []
            # 添加 项目筛选条件
            project_id = int(request.json.get("project_id", 0))
            # 没有实现双条件查询
            if project_id > 0:
        ...
        return RespModel.error_resp(msg='query_api_module_by_page 参数错误')


@module_route.route(f'/{module_name}/QueryById', methods=["GET"])
@JwtUtils.token_required
def query_api_module_by_id():
    try:
