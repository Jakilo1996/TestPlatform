...
import { ctApiProjectQueryByPage, ctApiProjectDeleteAction } from '~/api/api_test/ctApiProject'; // 不同页面不同的接口
...
const loadData = () => {
    ...
    ctApiProjectQueryByPage(searchData).then(
        (res: { data: { data; total: number; msg: string } }) => {
...
const onDelete = (index: number) => {
    ctApiProjectDeleteAction(tableData.value[index]["id"]).then((res: {}) => {
        loadData();
    });
};
